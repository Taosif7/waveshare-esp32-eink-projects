...
void EPD_BlitSprite(int16_t x, int16_t y, uint16_t w, uint16_t h, const uint8_t *bits) {
  if (bits == NULL || buffer == NULL || w == 0 || h == 0) {
    return;
  }
  const uint16_t stride = (uint16_t)((w + 7) / 8);
  for (uint16_t row = 0; row < h; row++) {
    const int16_t py = (int16_t)(y + (int16_t)row);
    if (py < 0 || py >= EPD_HEIGHT) {
      continue;
    }
    ...
    for (uint16_t col = 0; col < w; col++) {
      ...
      const uint8_t byte = src[col >> 3];
      const uint8_t mask = (uint8_t)(0x80 >> (col & 7));
      if (byte & mask) {
        EPD_DrawColorPixel((uint16_t)px, (uint16_t)py, DRIVER_COLOR_BLACK);
      }
    }
  }
}
